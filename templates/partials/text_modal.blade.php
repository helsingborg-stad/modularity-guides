...
<div class="grid mod-guide-text" {!! isset($item['toggle_key']) && !empty($item['toggle_key']) ? 'data-mod-guide-toggle-key-content="' . $item['toggle_key'] . '"' : '' !!}>
    <div class="grid-xs-12">
        <article>
            <h3><a href="#mod-guide-modal-{{ $modalId }}">{{ $content['title'] }}</a></h3>
            {!! $content['summary'] !!}
        </article>

        <div id="mod-guide-modal-{{ $modalId }}" class="modal modal-backdrop-2 modal-small" tabindex="-1" role="dialog" aria-hidden="true">
